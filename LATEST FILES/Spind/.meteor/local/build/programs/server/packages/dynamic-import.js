...
module.watch(require("./security.js"));                                                                         // 1
module.watch(require("./client.js"));                                                                           // 1
...
function read(pathParts, platform) {                                                                            // 47
  ...
  var absPath = pathNormalize(pathJoin.apply(undefined, [dynamicRoot].concat((0, _toConsumableArray3.default)(pathParts))));
                                                                                                                //
  if (!absPath.startsWith(dynamicRoot)) {                                                                       // 51
    throw new Meteor.Error("bad dynamic module path");                                                          // 52
  }                                                                                                             // 53
                                                                                                                //
  var cache = getCache(platform);                                                                               // 55
  return hasOwn.call(cache, absPath) ? cache[absPath] : cache[absPath] = readFileSync(absPath, "utf8");         // 56
}                                                                                                               // 59
                                                                                                                //
var cachesByPlatform = Object.create(null);                                                                     // 61
                                                                                                                //
function getCache(platform) {                                                                                   // 62
  return hasOwn.call(cachesByPlatform, platform) ? cachesByPlatform[platform] : cachesByPlatform[platform] = Object.create(null);
}                                                                                                               // 66
                                                                                                                //
process.on("message", function (msg) {                                                                          // 68
  // The cache for the "web.browser" platform needs to be discarded                                             // 69
  // whenever a client-only refresh occurs, so that new client code does                                        // 70
  // not receive stale module data from __dynamicImport. This code handles                                      // 71
  // the same message listened for by the autoupdate package.                                                   // 72
  if (msg && msg.refresh === "client") {                                                                        // 73
    delete cachesByPlatform["web.browser"];                                                                     // 74
  }                                                                                                             // 75
});                                                                                                             // 76
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
